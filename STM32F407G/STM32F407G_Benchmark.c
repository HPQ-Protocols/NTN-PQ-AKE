...
volatile uint32_t result_tls13_psk_kyber = 0; // Pre-Shared Key - PSK; TLS 1.3 PSK + Kyber
 ...
 * Pairwise-token version: no global S stored on endpoints
...
uint32_t benchmark_NTN_PQ_AKE_v40(void)
{
    ...
    /* ==========================================================
       Pairwise authorization seed

       Provisioning-time derivation, performed by TC/HSM only:
           T_AB = H(S_TC || ID_A || ID_B || pkA)

       Runtime endpoints DO NOT store S_TC.  Alice stores T_AB for
       Bob; Bob stores a verifier-table entry for (ID_A, pkA) -> T_AB.
       This benchmark models the one-pair case with the seed already
       provisioned locally.
       ========================================================== */
    uint8_t T_AB[32];
    memset(T_AB, 0x11, sizeof(T_AB));
    ...
    uint8_t nonceA[16];      /* serialized freshness tuple (r_A,n_A) */
    uint8_t nonceB[16];      /* serialized freshness tuple (r_B,n_B) */
    ...
    /************************************************************
     * M1 : Alice -> Bob
     * Alice sends (ID_A, pkA, nonceA, tokenA).  ID_A is implicit
     * in this benchmark; Bob uses (ID_A, pkA) to select T_AB.
     ************************************************************/
    start = DWT->CYCCNT;
    ...
    /* tokenA = MAC_{T_AB}(nonceA).  T_AB already binds ID_A, ID_B, pkA. */
    hmac_sha3_256(
        tokenA,
        T_AB,
        sizeof(T_AB),
    ...
    /************************************************************
     * M2 : Bob -> Alice
     * Bob verifies tokenA before expensive ML-KEM encapsulation.
     ************************************************************/
    start = DWT->CYCCNT;

    /* In a multi-device implementation, insert constant-time verifier
       table lookup here: (ID_A, pkA) -> T_AB. */
    hmac_sha3_256(
        tokenA_check,
        T_AB,
        sizeof(T_AB),
    ...
    if (memcmp(tokenA, tokenA_check, 32) != 0)
    {
        memset(skA,0,sizeof(skA));
        memset(tokenA_check,0,sizeof(tokenA_check));
        memset(T_AB,0,sizeof(T_AB));
        return 0;
    }
...
// =========================================================================
// ĐO LƯỜNG TLS 1.3 PSK + KYBER (Mô hình xác thực bằng Khóa chia sẻ trước)
// =========================================================================
void benchmark_TLS13_PSK_Kyber(void) {
    // Các biến phục vụ tầng Thỏa thuận khóa hậu lượng tử Kyber
    static uint8_t pk[CRYPTO_PUBLICKEYBYTES];
    static uint8_t sk[CRYPTO_SECRETKEYBYTES];
    static uint8_t ct[CRYPTO_CIPHERTEXTBYTES];
    static uint8_t ss_server[CRYPTO_BYTES], ss_client[CRYPTO_BYTES];

    // Các biến phục vụ tầng Xác thực & Trộn khóa (TLS 1.3 Key Schedule)
    static uint8_t psk[32];
    static uint8_t early_secret[32];
    static uint8_t handshake_secret[32];
    static uint8_t finished_key_server[32];
    static uint8_t finished_key_client[32];

    // Giả lập mã băm của các thông điệp bắt tay trước đó (Transcript Hash)
    static uint8_t transcript_hash[32] = {0x55, 0xAA, 0x11, 0x22};
    static uint8_t finished_tag_server[32];
    static uint8_t finished_tag_client[32];

    // Khởi tạo chuỗi khóa PSK mặc định đã nạp sẵn trong bộ nhớ thiết bị
    memset(psk, 0x22, sizeof(psk));

    uint32_t start, end;
    start = get_cycles(); // Sử dụng hàm đếm xung hệ thống của bạn

    // --- PHA 1: THỎA THUẬN KHÓA LƯỢNG TỬ (Kyber KEM) ---
    crypto_kem_keypair(pk, sk);          // Client sinh cặp khóa thành phần công khai
    crypto_kem_enc(ct, ss_server, pk);   // Server thực hiện đóng gói khóa (Encapsulate)
    crypto_kem_dec(ss_client, ct, sk);   // Client thực hiện mở gói khóa (Decapsulate)

    // --- PHA 2: TLS 1.3 PSK KEY SCHEDULE & XÁC THỰC (Dùng HMAC/HKDF) ---
    // 1. Giai đoạn HKDF-Extract tạo Early Secret dựa trên cấu hình khóa PSK
    hkdf_sha3(early_secret, psk, sizeof(psk), NULL, 0);

    // 2. Giai đoạn HKDF-Extract tiếp theo tạo Handshake Secret từ khóa Kyber vừa đồng thuận
    hkdf_sha3(handshake_secret, ss_server, CRYPTO_BYTES, early_secret, sizeof(early_secret));

    // 3. Server tính toán nhãn xác thực Finished (Server Finished MAC)
    // TLS 1.3 sẽ mở rộng Handshake Secret để tạo Finished Key, sau đó HMAC qua Transcript Hash
    hkdf_sha3(finished_key_server, handshake_secret, sizeof(handshake_secret), (uint8_t*)"server_finished", 15);
    hmac_sha3_256(finished_tag_server, finished_key_server, 32, transcript_hash, 32);

    // 4. Client tính toán nhãn xác thực Finished (Client Finished MAC) để Server đối chứng
    hkdf_sha3(finished_key_client, handshake_secret, sizeof(handshake_secret), (uint8_t*)"client_finished", 15);
    hmac_sha3_256(finished_tag_client, finished_key_client, 32, transcript_hash, 32);

    end = get_cycles();
    result_tls13_psk_kyber = end - start;
}
...
int main(void)
{
  ...
  // benchmark_TLS13_PSK_Kyber();

   // In kết quả ra màn hình Console/UART để lấy số liệu
   printf("Giao thức: TLS 1.3 PSK + Kyber\tCycles: %lu\n", result_tls13_psk_kyber);
